...
from utils.correlation import generate_filter_improved, mosse_filter_improved, generate_corr_response
from numpy.fft import fft2


class MosseTrackerImproved(Tracker):

    def name(self):
        return 'mosse_improved'
    
    def __init__(self, enlarge_factor, alpha, beta, gamma, sigma, adaptive_scale, learning_rate, psr_threshold):
        super().__init__()
    ...
    def initialize(self, image, region):
        ...
        self.window = round(max(region[2], region[3]) * self.enlarge_factor)
        ...
        # Calculate the filter and initialize the numerator and denominator
        patch = np.multiply(patch, self.cosine_window)
        f = fft2(patch)
        f_conj = np.conjugate(f)
        self.numerator = np.multiply(self.gaussian, f_conj)
        self.denominator = np.multiply(f, f_conj) + self.gamma
        self.filter = np.divide(self.numerator, self.denominator)
    ...
    def track(self, image):
        ...
        self.position, self.filter, self.numerator, self.denominator = mosse_filter_improved(image, self.position, (self.window, self.window), self.alpha, self.gamma, self.filter, self.cosine_window, self.gaussian, self.learning_rate, self.numerator, self.denominator, self.psr_threshold)
